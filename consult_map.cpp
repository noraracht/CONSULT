...
int main(int argc, char *argv[]) {
  ...
  for (uint64_t b_tmp = 4; b_tmp <= (20 * !given_b + given_b * 4); b_tmp++) {
    ...
    memory_usage_tmp = (4.0 * b_tmp * pow(2, 2 * h_tmp) * l + 8.0 * f_kmer_count) / pow(10.0, 9);
  ...
  try {
    ...
    cerr << "Failed to allocate memory for the encoding array-0." << ba.what() << endl;
  }
  try {
    encode_arr_1 = new uint64_t[encode_arr_size];
    cout << "Done memory allocation for the encoding array-1." << endl;
  } catch (bad_alloc &ba) {
    cerr << "Failed to allocate memory for the encoding array-1." << ba.what() << endl;
  }
